...
"""preprocess['nan'] = pd.isnull(preprocess['WORKED HOURS'])
for i in range(1,len(preprocess)):
    val = preprocess.iloc[i,4]
    if val == 'nan':
        val = kind_cat_dic[preprocess.iloc[i,3]]
    else:
        continue"""
...
#for key,value in kind_cat_dic.items():
#    preprocess.loc[preprocess['CATEGORY'] == key, :]['WORK HOURS'].fillna(value)
